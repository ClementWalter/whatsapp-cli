...
Single entry point exposing subcommands. Currently implemented:

- ``status``  — print whether a paired device exists on disk.
- ``login``   — connect, print a QR, and complete pairing if scanned.

Planned (stages 4+): ``chats``, ``read``, ``thread``, ``search``.
"""
...
# Make the sibling `wa/` package importable when invoked via `uv run <path>`.
